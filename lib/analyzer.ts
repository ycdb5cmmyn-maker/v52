  ...
  try {
    const response = await client.messages.create({
      ...
      messages: [
        {
          role: 'user',
          content: `Pieza tipo ${tipo} con scores: periodístico=${safePeriodistico.score} narrativo=${safeNarrativo.score} riesgo=${safeRiesgo.score} contextual=${safeContextual.score}. Problemas encontrados: ${parResults.filter(p => p.sev !== 'ok').length}. Veredicto editorial en 2 oraciones máximo.`,
        },
      ],
      system: execSys,
    });
